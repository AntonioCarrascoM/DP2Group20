...
public interface MemberRepository extends JpaRepository<Member, Integer> {
	...
	@Query("select m from Member m where (select count (r1) from Member m1 join m1.requests r1 where r1.status='1' and m1.id=m.id)*1.0 > (select avg((select count(r2) from Request r2 where r2.status='1' and r2.member.id=m3.id)*1.0) from Member m3)*1.1")
