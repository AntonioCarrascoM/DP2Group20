...
public class SocialProfileTest extends AbstractTest {
	...
	public void SocialProfilePositiveTest() {
		final Object testingData[][] = {
			...
			{
				"member1", null, null, "create", null
			}
		/*
		 * Positive test: A actor(member) registers his socialProfile.
		 * Requisite tested: Functional requirement - 11.1 An actor who is authenticated as a member must be able to manage his or her socialProfiles to march on a procession, which includes listing them by status, showing, creating them, and deleting
		 * them.
		 * Data coverage : We created a socialProfile by providing 3 out of 3 editable attributes.
		 * Exception expected: None. A actor (Member) can create socialProfiles.
		 */
		};
	...
	protected void template(final String username, final String st, final String id, final String operation, final Class<?> expected) {
		...
		try {
			...
			if (operation.equals("create")) {
				...
				s.setActor(this.memberService.findOne(this.getEntityId(username)));
